...
class UIDot(UIElement):
    def __init__(self, position, color, bg_color=None):
        ...
        #pygame.draw.line(self.surf, color, (0, 5), (10, 5))
        #pygame.draw.line(self.surf, color, (5, 0), (5, 10))
        pygame.draw.rect(self.surf, color, (0, 6, 15, 3))
        pygame.draw.rect(self.surf, color, (6, 0, 3, 15))
    ...
    def __init__(self, ui, gamestate):
        ...
        for pos, t in self.gamestate.grid.items():
            if t == PaperRacePointType.Street:
                self.sprites[pos] = UIDot(self.ui.coord_game2ui(pos), (50,50,50))
            elif t == PaperRacePointType.Block:
                #self.sprites[pos] = UIDot(self.ui.coord_game2ui(pos), (0,0,0), (0,0,0))
                pass
            else:
                self.sprites[pos] = UIDot(self.ui.coord_game2ui(pos), (150,0,0))
...
class UIRacer(UIElement):
    ...
    def angle_between(self, d1, d2):
        angle = {(0,1): 180, (0,-1): 0, (1,0): -90, (-1,0):90, (1,-1): 45, (-1,-1): -45, (-1,1): -135 ,(1,1): 135}
...
class UIRacerLayer(UIListLayer):
    def __init__(self, ui, gamestate):
        ...
        for id, racer in self.gamestate.racer.items():
            color = (random.randrange(0,5)*50, random.randrange(0,5)*50, random.randrange(0,5)*50)
            self.sprites[id] = UIRacer(self.ui.coord_game2ui(racer.position), racer, color)

    def update(self):
        for id, racer in self.sprites.items():
            racer.update(self.ui.coord_game2ui(self.gamestate.racer[id].position))
...
class UICurrentRacer(UIListLayer):
    ...
    def update(self):
        ...
        # update sprites
        self.sprites = {
            p: UINextPositionMarker(self.ui.coord_game2ui(p), (200,0,0))
...
class UserInterface:
    ...
    def init_game(self):
        ...
        #self.gamestate.grid.load_from_bitmap("map2.png")
    ...
    def coord_game2ui(self, position):
        g2u = lambda x: x*self.cell_size + self.cell_size//2 + self.padding
        return Coord(map(g2u, position))

    def coord_ui2game(self, position):
        u2g = lambda x: (x - self.padding) // self.cell_size
        return Coord(map(u2g, position))

    def processInput(self):
        
        # mouse hover events
        #self.mouse_hover_sprites = [s for s in sprites if s.rect.collidepoint(pos)]
        #pygame.event.wait()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                self.mouse_ui_pos = pygame.mouse.get_pos()  # position of the mouse in pixel
                self.mouse_pos = self.coord_ui2game(self.mouse_ui_pos)  # position of the mouse in game coordination
